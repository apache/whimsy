...
# $1 = base, $2 = extension
ARTIFACT_RE = %r{/([^/]+\.(pom|tar|tar\.xz|tar\.gz|deb|nbm|dmg|sh|zip|tgz|tar\.bz2|jar|whl|war|msi|exe|rar|rpm|nar|xml))([&?]action=download)?$}
...
#                 Apache_OpenOffice-SDK_4.1.10_Linux_x86-64_install-deb_en-US
  ...
  links.each do |h, t|
    ...
    if h =~ %r{^https?://.+?/([^/]+\.(asc|sha\d+|md5|sha|mds))$}
      ...
      next if %w(sha256 md5 mds sha512 sha1).include?(ext) and %w(SHA digest Digest).include?(t) # generic
