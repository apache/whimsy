#!/usr/bin/env ruby
PAGETITLE = "Update nominated-members.txt" # Wvisible:members
# Note: PAGETITLE must be double quoted

# Page to allow arbitrary updates to nominated-members.txt, without needing to install or know SVN
...
def process_form(source, env, formdata: {})
  ...
  begin
    _p class: 'system' do
      ASF::SVN.updatefile(source, "Update my nomination", env, _, formdata['revision']) do |_original|
        formdata['updated']
      end
    end
    return true
